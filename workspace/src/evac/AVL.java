...
public class AVL<Key extends Comparable<? super Key>, Value>
{
  ...
  class Min { 
	...
	public Min (Key key, Value value) { 
	      this.key = key;
	      this.value = value;
	} 
  ...
  // Get min --- fais par nous 
  // BIEN MAIS PROBLEME DU NUL POINTER ! 
  // A VOIR POUR INSTANCIATION DE REP en executant voit erreur
  public Min getMin (Node node) {
	  Min rep = null; 
	  if (node==null) { 
		  // 
	  }
	  ...
	  if (node.left == null) {
		rep.key = node.left.key ;
		rep.value = node.left.value ; 
	 }
	 else { 
		 rep = getMin(node.left) ; 
	 }
	  return rep ; 
  } 
  ...
  public static void main(String[] args) {
    ...
    AVL.Min tchaz = tree.getMin(tree.root); 
    System.out.println("======TEST MIN ====" + tchaz);
    // ça m'a branlé !!!!!!
